...
env.user = 'root'
env.sudo_user = 'root'
env.hosts = '39.108.209.163'

db_user = 'root'
db_password = 'SHAWling123'

_TAR_FILE = 'dist-stams.tar.gz'

_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE

_REMOTE_BASE_DIR = '/srv/stams'
...
def backup():
    ...
    '''
    dt = _now()
    f = 'backup-stams-%s.sql' % dt
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick stams > %s'
...
def build():
    ...
    Build dist package.
    '''
    includes = ['handlers', 'resources', 'templates', '*.py']
...
def deploy():
    ...
    with settings(warn_only=True):
        sudo('supervisorctl stop stams')
        sudo('supervisorctl start stams')
